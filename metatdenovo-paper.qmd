...
pcas <- list()
for ( r in c(
  'megahit.prodigal.without_bbnorm', 'rnaspades.prodigal.without_bbnorm', 
  'megahit.prokka.without_bbnorm',   'rnaspades.prokka.without_bbnorm'
  ) 
) {
...
#| tbl-cap: '**Overall statistics for MST-1 assemblies.** *) The assembly was made from read-normalized data. †) Contigs shorter than 200 nucleotides were removed using the --min_contig_length parameter.'

transrates %>%
  filter(project == 'mst-1') %>%
  filter(! ( assembler == 'rnaspades' & minlen == 0) ) %>%
  inner_join(
    overall_stats %>% 
...
p0 <- eukulele %>%
  ...
  mutate(name = sprintf("%s: %s", orf_caller, var)) %>%
  ggplot(aes(assembly, value, fill = taxon_rank)) +
  geom_col() +
...
p1 <- func_annots %>%
  ...
  mutate(var = factor(var, levels = c('N. ORFs', 'count', 'tpm'), ordered = TRUE)) %>%
  mutate(name = sprintf("%s: %s", orf_caller, var)) %>%
  ggplot(aes(assembly, value, fill = combination)) +
  geom_col() +
...
### Functional and Taxonomical annotation - comparison with original data
...
p2 <- pcas[['rnaspades.prokka.without_bbnorm-families']][['CA']] %>% plot_pca('RNASpades/Prokka')
p3 <- pcas[['rnaspades.prodigal.without_bbnorm-families']][['CA']] %>% plot_pca('RNASpades/Prodigal')
...
p2 <- pcas[['rnaspades.prokka.without_bbnorm-eggnogs']][['CA']] %>% plot_pca('RNASpades/Prokka')
p3 <- pcas[['rnaspades.prodigal.without_bbnorm-eggnogs']][['CA']] %>% plot_pca('RNASpades/Prodigal')
...
```{r mst rnaspades prodigal - functional annotation table - selected genes, echo = FALSE, warning = FALSE}
...
(p3 / (p5 + p6) ) / p7 + plot_annotation(tag_levels = 'A')
```
